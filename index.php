	...
	if($_GET["action"] == "delete")
	{
		foreach($_SESSION["shopping_cart"] as $keys => $values)
		{
			if($values["item_id"] == $_GET["id"])
			{
				...
				echo "<script>window.location='index.php'</script>"; //refresh page but retains the session variables
			}
		}
	}
}
...
<html>
	<head>
		<title>PHP Mysql Shopping Cart</title>
	...
	<body>
		...
		<div class="container">
			<div class="row">
				<?php
				//Get the items to sell from sql db
				...
				if($result->num_rows > 0)
				{
					...
					{
				?>
						<!-- For each item, they live in a dedicated form -->
						<form class="col" method="post" action="index.php?action=add&id=<?php echo $row["id"]; ?>">
